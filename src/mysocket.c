...
/*Read the line from socket until you reach last */
int ReadUntil(char *ptr, char objective, char* read) {

//  printf("Entrou na ReadUntil\n");
//  printf("ptr é %s\n",ptr );

  char* location = strchr(ptr,objective); // finds where the first objective is in ptr
  int stop = location - ptr; // How much chars there is until objective
  strncpy(read,ptr,(size_t)stop); //Copy to read #stop characters from ptr
//  printf("Read é %s\n",read);
  return stop;
}